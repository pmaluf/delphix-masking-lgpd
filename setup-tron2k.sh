          ...
          if [[ ! ${CONNECTORNAME} =~ "#" ]]
            then
              ...
              do
                ...
                if [ ${COUNT} -lt 1000 ]
                 then
                   ret=$(create_tablemetadata ${TABLE} 177 ${DATABASETYPE})
                elif [ ${COUNT} -ge 1000 -a ${COUNT} -lt 2000 ]
                  then
                    ret=$(create_tablemetadata ${TABLE} 178 ${DATABASETYPE})
                elif [ ${COUNT} -ge 2000 -a ${COUNT} -lt 3000 ]
                  then
                    ret=$(create_tablemetadata ${TABLE} 179 ${DATABASETYPE})
                elif [ ${COUNT} -ge 3000 -a ${COUNT} -lt 4000 ]
                  then
                     ret=$(create_tablemetadata ${TABLE} 180 ${DATABASETYPE})
                elif [ ${COUNT} -ge 4700 ]
                  then
                    ret=$(create_tablemetadata ${TABLE} 181 ${DATABASETYPE})
                fi
                let "COUNT++"
              done
